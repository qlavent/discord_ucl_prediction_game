...
class SelectMatch(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"Match **{self.id_to_home_team[selected_match_id]} vs {self.id_to_away_team[selected_match_id]}** selected. Please select {self.id_to_home_team[selected_match_id]}'s goals.",
            view=view
        )
...
class SelectGoals(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.placeholder == f"{self.parent_view.id_to_home_team[self.parent_view.match_id]}":
            ...
            await interaction.response.send_message(f"{self.parent_view.id_to_home_team[self.parent_view.match_id]} goals set to {home_goals}. Now select the {self.parent_view.id_to_away_team[self.parent_view.match_id]}'s goals.", view=view)
            ...
            await interaction.response.send_message(f"Prediction saved: {self.parent_view.id_to_home_team[self.parent_view.match_id]} {home_goals} - {away_goals} {self.parent_view.id_to_away_team[self.parent_view.match_id]}.")

def show_upcomming_matches(next_matchday_matches, user_id):
